...
vtkSmartPointer<vtkPolyData> MyReadPolyData(const char* fileName)
{
  ...
  // DO ALL in POINTS
    for (vtkIdType i = 0; i < numPoints; ++i) {
        ...
        if (pointColorArray && pointColorArray->GetNumberOfComponents() >= 3) {
            unsigned char rgba[4] = {255, 255, 255, 255};
            pointColorArray->GetTypedTuple(i, rgba);
            float r = rgba[0] / 255.f;
            float g = rgba[1] / 255.f;
            float b = rgba[2] / 255.f;
            float a = (pointColorArray->GetNumberOfComponents() == 4) ? (rgba[3] / 255.f) : 1.0f;
            color[0] = r;
            color[1] = g;
            color[2] = b;
    ...
    for (cells->InitTraversal(); cells->GetNextCell(npts, ptIds);++cellId) {
        if (npts == 3) { // only triangled polys
           // 5-1 TRIANGLES
            ...
            // 5-3 CELL COLOR 
            FVector color(vtkVector3<float>(0.5,0.5,0.5));
            if (!pointColorArray && cellColorArray && cellColorArray->GetNumberOfComponents() >= 3) {
                unsigned char rgba[4] = {255, 255, 255, 255};
                cellColorArray->GetTypedTuple(cellId,rgba);
                float r = rgba[0] / 255.f;
                float g = rgba[1] / 255.f;
                float b = rgba[2] / 255.f;
                float a = (pointColorArray->GetNumberOfComponents() == 4) ? (rgba[3] / 255.f) : 1.0f;
